...
partial class SayehColumnResizeOptions<TItem> where TItem : class
{
    private string? _width;

    [CascadingParameter]
    internal InternalGridContext<TItem> InternalGridContext { get; set; } = default!;

    /// <summary>
    /// Gets a reference to the enclosing <see cref="FluentDataGrid{TGridItem}" />.
    /// </summary>
    private SayehDataGrid<TItem> Grid => InternalGridContext.Grid;
